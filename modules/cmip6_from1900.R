...
saveloc = "./data/cmip6/all_ctemp_calced_1900_plus_world.Rdata"
if(file.exists(saveloc)){
  ...
  for (f in files) {
    ...
    temp = read_feather(f, as_data_frame = FALSE)
    ...
    desiredCols = c(colnames(temp)[1:11], sapply(c(1900:2105), toString))
    temp2 = as.data.table(temp[, desiredCols])
    ctemp = melt(temp2, id.vars=c("region", "scenario", "climate_model"), measure.vars=paste(1900:2105),
                 variable.name="year",value.name="temperature",
                 variable.factor = F)
    
    print("File loaded")
    # Note that this is not population weighted, but this string will be removed later
    ctemp$region[ctemp$region=="Nearest World"] = "Popn weighted World|Natural Earth 50m|Global average"

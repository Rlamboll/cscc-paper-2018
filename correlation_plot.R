...
# load GDP from 2000 for all SSP scenarios
...
global_damages <- tempandgdp_global[, damages := gdp * 0.00236 * (temp-basetemp)^2]


# Damage = gdp_w * a_w * (T_w – T_w(0))^2 = \sum a_c * gdp_c * (T_c-T_c(0))^2
# sum country damages for each model and rcp in each year
summed_damages <- country_damages %>% group_by(year, rcp, model, ssp) %>% summarize(damages = sum(damages, na.rm=T), gdp = sum(gdp, na.rm=T))
